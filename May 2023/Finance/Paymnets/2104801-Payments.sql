...
select
    ...
    concat(
        "https://erp.floranow.com/payment_transactions/",
    ...
    py.credit_note_id,
    py.payment_type,
    ----over_payed, credit, write_off, cheque, payment_by_credit, bank_transfer, visa_card, cash, null
    py.currency,
    py.added_by,
    py.approved_by,
    pt.collected_by,
...
from
    ...
    left join `floranow.Floranow_ERP.users` as stg_users on stg_users.id = i.customer_id --left join `floranow.erp_prod.move_items` as mi on mi.id = py.debit_move_item_id 
    ...
    left join `floranow.erp_prod.bank_accounts` as ba on stg_users.bank_account_id = ba.id --where i.number = 'F10122712'