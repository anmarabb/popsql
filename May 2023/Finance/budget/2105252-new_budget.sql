...
SELECT 
  ...
  CASE
        WHEN ROW_NUMBER() OVER (PARTITION BY DATE_TRUNC(date, MONTH)) = 1 THEN 
            max(case when DATETIME_DIFF(DATETIME_SUB(DATETIME_ADD(DATETIME_TRUNC(d,MONTH), INTERVAL 1 MONTH), INTERVAL 1 DAY),d,DAY)-1 >=0 then DATETIME_DIFF(DATETIME_SUB(DATETIME_ADD(DATETIME_TRUNC(d,MONTH), INTERVAL 1 MONTH), INTERVAL 1 DAY),d,DAY)-1 else null end) OVER (PARTITION BY DATE_TRUNC(date, MONTH))
        ELSE 
            NULL 
    END AS days_remaining,

ROW_NUMBER() OVER (PARTITION BY financial_administration,DATE_TRUNC(date, MONTH),DATE_TRUNC(date, day)) as row_num,
