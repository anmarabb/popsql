with base as(
                        ...
                        group by 1,2,3
                            ...
                            select
                                ...
                                date(DATE_TRUNC(i.printed_at,month)) as month_of_year,
...
round(avg(monthly_revenue)) as avg_mrr,
round(max(monthly_revenue)) as max_mrr,
round(min(monthly_revenue)) as min_mrr,
round(avg(deliveries)) as avg_deliveries,


round(avg(clients)) as avg_clients,

round(avg(frequency)) as avg_frequency,
round(avg(basket_size)) as avg_basket_size,
round(avg(items)) as avg_items,
round(avg(quantity)) as avg_quantity,


from base
where  month_of_year>='2022-01-01' and month_of_year < DATE_TRUNC(current_date(),month) and financial_administration != 'Internal'
