...
stg_move_items.aging_date,
    ...
    case when date_diff( cast(current_date() as date ),cast(stg_move_items.aging_date as date), DAY) > 30 and date_diff( cast(current_date() as date ),cast(i.printed_at as date), DAY) <= 60 then i.remaining_amount else 0 end as between_31_to_60_days,
