...
stg_move_items.aging_date,
    ...
    case when date_diff( cast(current_date() as date ),cast(stg_move_items.aging_date as date), DAY) > 90 and date_diff( cast(current_date() as date ),cast(stg_move_items.aging_date as date), DAY) <= 120 then i.remaining_amount else 0 end as between_91_to_120_days,
