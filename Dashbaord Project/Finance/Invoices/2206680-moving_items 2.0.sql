...
case when date_diff( cast(current_date() as date ),cast(case when i.id is not null then date(i.due_date) else date(mi.date) end as date), DAY) <= 30 then stg_move_items.residual else 0 end as up_to_30_days,
