...
case 
    ...
    when max(orr.standing_order_id) is not null)  and DATE_DIFF(CAST(CURRENT_DATE() AS date), CAST(MAX(i.printed_at) AS date),day ) > 7 and DATE_DIFF(CAST(CURRENT_DATE() AS date), CAST(MAX(i.printed_at) AS date),day ) <= 30 then 'inactive'
    when max(orr.standing_order_id) is not null)  and DATE_DIFF(CAST(CURRENT_DATE() AS date), CAST(MAX(i.printed_at) AS date),day ) > 30 then 'churned'
    else 'churned'  
...
where ii.status = 'APPROVED' and ii.deleted_at is null and ii.customer_id  =1385
