...
case 
    when date_diff(cast(current_date() as date ),cast(max(u.created_at) as date), MONTH) <8 and DATE_DIFF(CAST(CURRENT_DATE() AS date), CAST(MAX(li.created_at) AS date),day ) <= 7 then 'new_active' 
        when date_diff(cast(current_date() as date ),cast(max(u.created_at) as date), MONTH) <8 and DATE_DIFF(CAST(CURRENT_DATE() AS date), CAST(MAX(li.created_at) AS date),day ) > then 'new_inactive'  
