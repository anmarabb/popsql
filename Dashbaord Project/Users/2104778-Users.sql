...
case 
    ...
    when DATE_DIFF(CAST(CURRENT_DATE() AS date), CAST(MAX(li.created_at) AS date),day ) < 8 or stg_invoice_items_customer_id.manual_account_status in ('active') or or stg_invoice_items_customer_id.standing_order_account_status in ('active')then 'active'
