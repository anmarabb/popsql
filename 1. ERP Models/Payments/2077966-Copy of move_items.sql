with prep_payments as ( select credit_move_item_id, max(py.created_at) as payment_date, sum(paid_amount) as paid_amount from `floranow.erp_prod.payments` as py group by 1)
...
left join `floranow.erp_prod.invoices` as i on moi.documentable_id = i.id and documentable_type = 'Invoice'
...
select 
pt_moi.id,
ARRAY_AGG(i.number) as invoice_numbers,


from `floranow.erp_prod.move_items` as pt_moi 
join `floranow.erp_prod.payments` as py on pt_moi.id=py.credit_move_item_id 
join  `floranow.erp_prod.move_items` as inv_moi on inv_moi.id=py.debit_move_item_id 
join `floranow.erp_prod.invoices` as i on inv_moi.documentable_id = i.id and inv_moi.documentable_type = 'Invoice'

where pt_moi.entry_type ='CREDIT' and pt_moi.documentable_type = 'PaymentTransaction' and i.number is not null
group by pt_moi.id
/*
