...
count(distinct i.invoice_id) as row_coun,
...
where ii.deleted_at is null and  ii.__hevo__marked_deleted is not true and EXTRACT(YEAR FROM i.printed_at) = EXTRACT(YEAR FROM DATE_SUB(CURRENT_DATE(), INTERVAL 1 MONTH)) and i.financial_administration_id = 1 and ii.status = 'APPROVED' and warehouse = 'Qassim Warehouse'