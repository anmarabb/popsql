...
select 
  p.id,
    ...
    EXTRACT(DAY FROM (CAST(p.expired_at AS DATE) - CAST(p.updated_at AS DATE))) - 
    EXTRACT(DAY FROM (CAST(CURRENT_DATE AS DATE) - CAST(p.updated_at AS DATE))) AS Remaining_Age,
