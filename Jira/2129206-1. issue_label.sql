SELECT
    issue_label.*,
FROM
    `floranow.floranow_jira.issue` AS issue
...
WITH
    prepared_labels AS (
        SELECT
            ...
            MAX(
                CASE
                    WHEN rn = 1 THEN label
                END
            ) AS label_1,
            MAX(
                CASE
                    WHEN rn = 2 THEN label
                END
            ) AS label_2,
            MAX(
                CASE
                    WHEN rn = 3 THEN label
                END
            ) AS label_3,
            MAX(
                CASE
                    WHEN rn = 4 THEN label
                END
            ) AS label_4
        FROM
            (
                ...
                FROM
                    `floranow.floranow_jira.issue_label`
            )
        GROUP BY
            issue_id
    )
